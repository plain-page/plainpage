// ---- Book info modal: metadata display/edit, Open Library lookup ----
...
function renderBookInfoCover(book) {
  book.cover ? (infoCoverImg.src = book.cover, infoCoverImg.hidden = !1, infoCoverPlaceholder.hidden = !0) : (infoCoverImg.hidden = !0, infoCoverImg.src = "", infoCoverPlaceholder.hidden = !1)
}

// Both now return a promise resolving to true/false, so callers that need
// to know whether the write actually landed (e.g. to revert an optimistic
// UI change on failure) can await it, instead of getting a value that was
// always true regardless of what IndexedDB did.
function persistBookMetadataSafe(book) {
  if (!book) return Promise.resolve(!1);
  if (currentBook && book.id === currentBook.id) return currentBook.description = book.description, currentBook.published = book.published, currentBook.status = book.status, currentBook.cover = book.cover, persistCurrentBookSafe();
  var list = loadLibraryList(),
    idx = list.findIndex(function(b) {
      return b.id === book.id
    });
  return -1 !== idx ? (list[idx] = book, saveLibraryList(list)) : Promise.resolve(!1)
}

function persistCurrentBookSafe() {
  if (!currentBook) return Promise.resolve(!1);
  var list = loadLibraryList(),
    idx = list.findIndex(function(b) {
      return b.id === currentBook.id
    });
  return -1 !== idx ? (list[idx] = currentBook, saveLibraryList(list)) : Promise.resolve(!1)
}
infoCoverPreview.addEventListener("click", function() {
  ...
  if (file && bookInfoTarget) {
    ...
    reader.onload = function(ev) {
      downscaleImageDataUrl(ev.target.result, 720, .9).then(function(url) {
        var previousCover = book.cover;
        book.cover = url, persistBookMetadataSafe(book).then(function(ok) {
          if (!ok) return book.cover = previousCover, infoCoverInput.value = "", void showToast("That image is too large to save — try a smaller file.");
...
infoFetchBtn.addEventListener("click", function() {
  ...
  if (bookInfoTarget) {
    ...
    if (window.confirm('Delete "' + title + '"? This can\u2019t be undone.')) {
      ...
      saveLibraryList(list), idbDeleteBookContent(book.id);
